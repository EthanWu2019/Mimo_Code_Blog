...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.url) {
        await fetch('/api/user/profile', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, avatar: data.url }) });
        setProfile((p: any) => ({ ...p, avatar: data.url }));
        await updateSession();
      }
    } catch {}
  };
